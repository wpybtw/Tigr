...
int main(int argc, char **argv)
{
	...
	gpuErrorcheck(cudaMallocManaged(&d_nodePointer, num_nodes * sizeof(unsigned int)));
	gpuErrorcheck(cudaMallocManaged(&d_edgeList, (num_edges + num_nodes) * sizeof(unsigned int)));

	gpuErrorcheck(cudaMalloc(&d_finished, sizeof(bool)));
	gpuErrorcheck(cudaMallocManaged(&d_partNodePointer, vGraph.numParts * sizeof(PartPointer)));

	gpuErrorcheck(cudaMemAdvise(d_nodePointer, num_nodes * sizeof(uint), cudaMemAdviseSetPreferredLocation, arguments.deviceID));
	gpuErrorcheck(cudaMemAdvise(d_edgeList, (num_edges + num_nodes)  * sizeof(uint), cudaMemAdviseSetPreferredLocation, arguments.deviceID));
	gpuErrorcheck(cudaMemAdvise(d_partNodePointer, vGraph.numParts * sizeof(PartPointer), cudaMemAdviseSetPreferredLocation, arguments.deviceID));

	memcpy(d_nodePointer, vGraph.nodePointer, num_nodes * sizeof(unsigned int));
	memcpy(d_edgeList, vGraph.edgeList, (num_edges + num_nodes) * sizeof(unsigned int));
	memcpy(d_partNodePointer, vGraph.partNodePointer, vGraph.numParts * sizeof(PartPointer));
